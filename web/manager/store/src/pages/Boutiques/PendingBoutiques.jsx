    ...
    <div className="dashboard-page">
      ...
      <div className="content-with-sidebar">
        ...
        <div className="review-panel">
          <h4>Queued Highlights</h4>
          <p className="panel-subtitle">Oldest requests bubble to the top</p>
          <div className="queue-list">
            {queueHighlights.length === 0 && (
              <div className="queue-placeholder">Queue is clear 🎉</div>
            )}
            {queueHighlights.map((request) => {
              const stageKey = getStageKey(request.status);
              const age = getAgeInDays(request.createdAt);
              return (
                <div key={request._id} className="queue-item">
                  <div className="queue-item-header">
                    <span className="queue-item-title">{request.name || 'Unnamed Boutique'}</span>
                    <span className={`stage-pill compact ${stageKey}`}>
                      {STAGE_LABELS[stageKey]}
                    </span>
                  </div>
                  <div className="queue-item-meta">
                    <span>{request.ownerName || 'Unknown owner'}</span>
                    <span>•</span>
                    <span>{age} day{age === 1 ? '' : 's'} open</span>
                  </div>
                  <button
                    type="button"
          ...
          <div className="panel-insights">
            ...
            <ul>
              <li>
                {stats.awaiting} awaiting initial triage
              </li>
              <li>
                {stats.overdue} over 7 days old
              </li>
              <li>
                {stats.needsInfo} requesting more info
              </li>
            </ul>
          </div>
        </div>
...
export default PendingBoutiques;
