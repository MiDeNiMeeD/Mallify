...
const VerifiedBoutiques = () => {
  ...
  const growthStories = useMemo(() => {
    return filteredBoutiques
      .filter((item) => getHealthBucket(item) === 'steady')
      .slice(0, 3);
  }, [filteredBoutiques]);
    ...
    <div className="dashboard-page">
      ...
      <div className="content-with-sidebar">
                        ...
                        <tr key={boutique._id}>
                          <td>
                            <div className="table-primary">
                              ...
                              <span className="table-subtitle">{boutique.email || 'No email provided'}</span>
                            </div>
                          </td>
                          <td>
                            <div className="table-primary">
                          ...
                          <td>
                            <div className="table-metric">
                              ...
                              <small>{boutique.totalSales ? 'Lifetime' : 'New listing'}</small>
                            </div>
                          </td>
                          <td>
                            <div className="table-metric">
                          ...
                          <td>
                            <span className={`health-pill ${health}`}>
                              ...
                              {health === 'steady' && 'Steady'}
                              {health === 'watch' && 'Watchlist'}
                            </span>
                          </td>
                          <td>
                            <button
                              ...
                              style={{ fontSize: '0.75rem', padding: '0.4rem 0.85rem', display: 'inline-flex', gap: '0.35rem', alignItems: 'center' }}
        ...
        <div className="insights-panel">
          <h4>Growth leaderboard</h4>
          <p className="panel-subtitle">Top performers ranked by lifetime sales</p>
          <div className="leaderboard-list">
            {leaderBoard.length === 0 && <div className="queue-placeholder">No verified boutiques yet</div>}
            {leaderBoard.map((boutique, index) => (
              <div key={boutique._id} className="leaderboard-item">
                <div className="leaderboard-rank">#{index + 1}</div>
                <div className="leaderboard-details">
                  <span className="leaderboard-name">{boutique.name || 'Unnamed Boutique'}</span>
                  <span className="leaderboard-meta">
                    {formatCurrency(getSalesValue(boutique))} • {formatNumber(getOrdersValue(boutique))} orders
                  </span>
                </div>
                <button
                  type="button"
          ...
          <div className="growth-stories">
            <h5>Steady growers</h5>
            {growthStories.length === 0 && <div className="queue-placeholder">No steady growth stories</div>}
            {growthStories.map((boutique) => (
              <div key={boutique._id} className="growth-card">
                <div>
                  <div className="growth-title">{boutique.name || 'Unnamed Boutique'}</div>
                  <div className="growth-meta">{boutique.ownerName || 'Unknown owner'}</div>
                </div>
...
export default VerifiedBoutiques;
