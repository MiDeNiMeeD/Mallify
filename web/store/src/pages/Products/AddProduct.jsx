...
import { Plus, Upload, X, Save, FileText, Loader2, Check } from 'lucide-react';
...
const getColorLabelByHex = (hex) => {
  ...
  const match = COLOR_OPTIONS.find((option) => option.value.toLowerCase() === normalized);
...
function AddProduct() {
  ...
  const variantInventoryTotal = variants.reduce((sum, variant) => {
    const sizeTotal = (variant.sizes || []).reduce(
      (sizeSum, sizeRow) => sizeSum + Math.max(0, Math.round(toNumber(sizeRow.quantity))),
      0
    );
    ...
    setVariants((prev) =>
      prev.map((variant) => {
        ...
        const existingLabels = new Set(
          (variant.sizes || []).map((sizeRow) => sizeRow.label.trim().toLowerCase()).filter(Boolean)
        );
  ...
  const resetForm = () => {
    ...
    setVariants([]);
    setAttributeRows(getInitialAttributeRows());
    setImages([]);
  };
  ...
  const submitProduct = async (statusOverride = formData.status) => {
    ...
    if (!boutiqueId) {
      showToast('You need an approved boutique before creating products.', 'error');
      return;
    }
      ...
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'nowrap', minWidth: 'fit-content' }}>
          <button
            ...
            aria-label={isSubmitting ? 'Saving product' : 'Create product'}
          >
