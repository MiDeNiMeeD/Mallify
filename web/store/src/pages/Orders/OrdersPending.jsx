...
import { AlertCircle, CheckCircle, XCircle, Eye, Package, ChevronDown, ChevronUp, DollarSign, Clock3, Search, Filter, Copy, Download } from 'lucide-react';
...
function OrdersPending() {
  ...
  const handleReject = async (orderId) => {
    confirmAction({
      message: 'Reject this order? A note is required.',
      confirmLabel: 'Reject Order',
      variant: 'danger',
      requireNote: true,
      onConfirm: async (note) => {
        try {
          await apiClient.updateStoreOrderAction(orderId, 'reject', note);
          setOrders((prev) => prev.filter(o => o._id !== orderId));
  ...
  const pendingOrders = orders.filter(
    (order) => normalizeStoreOrderStatus(order.status) === 'pending'
  );
  const totalItems = pendingOrders.reduce((sum, order) => sum + orderItemsCount(order), 0);
  const totalValue = pendingOrders.reduce((sum, order) => sum + orderAmount(order), 0);
  const dayMs = 24 * 60 * 60 * 1000;
  const pendingToday = pendingOrders.filter((order) => {
    const created = order?.createdAt ? new Date(order.createdAt).getTime() : 0;
    return created && Date.now() - created <= dayMs;
  }).length;
  ...
  const handleBulkReject = () => {
    confirmAction({
      message: `Reject ${selectedOrderIds.length} selected orders? A note is required.`,
      confirmLabel: 'Reject Selected',
      variant: 'danger',
      requireNote: true,
      onConfirm: (note) => {
        runBulkAction('reject', note);
      },
    });
  };
  ...
  const exportFilteredOrdersToCsv = () => {
    ...
    link.href = url;
    link.download = `pending-orders-${new Date().toISOString().slice(0, 10)}.csv`;
    link.click();
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card">
          <div className="stat-card-header">
            ...
            <div className="stat-icon pink"><Clock3 size={20} /></div>
          </div>
      ...
      <div className="content-card">
        <div className="card-header">
          <div className="orders-card-title-wrap">
            <AlertCircle size={20} className="orders-card-title-icon warning" />
          ...
          <div className="orders-toolbar">
            {selectedOrderIds.length > 0 && (
              <div className="bulk-actions-bar">
                ...
                <button className="btn-secondary btn-action-confirm" onClick={() => runBulkAction('confirm')}>Accept Selected</button>
                <button className="btn-secondary btn-action-reject" onClick={handleBulkReject}>Reject Selected</button>
              </div>
            )}
          </div>
